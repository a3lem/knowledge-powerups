...
-  /knowledge-powerups:docs-folder
...
Codes are random, so there is no registry to maintain. Generate them with the bundled generator, `scripts/gen-spec-codes.py` next to this SKILL.md, which prints one code per line:

```
<skill-base-dir>/scripts/gen-spec-codes.py -k 3
```
...
Specs belong in `docs/`. Name of the spec is in the file name, not folder name. Specs are identified by their context. Reference specs belong in `docs/specs/`. Spec deltas are found in `docs/wip/<work-item>/specs/`. Deltas are marked with a `.delta.` infix in the name.
...
A delta for a planned change (work item `harden-signup`) to the spec above, at `docs/wip/harden-signup/specs/user-registration.delta.md`. The file name identifies the target spec, so the delta contains only the operations:
