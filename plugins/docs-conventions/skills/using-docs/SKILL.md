...
The split is also a writing rule. Outside `dev/`, state observed fact only -- what has been verified against the code. Inside `dev/`, hypotheses and assumptions are welcome, but frame them as such ('probably', 'assuming X', 'unverified'): a future reader cannot tell a guess from an observation unless the words carry the uncertainty. It follows that promotion out of `dev/` requires re-verification against the code, unless the human explicitly waives it.
...
./  # repository root
  ...
  docs/
    ...
    adrs/  # optional. decision records (see /adrs skill)
