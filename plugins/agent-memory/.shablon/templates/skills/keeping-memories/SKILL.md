...
Two size caps are enforced, not advisory: {{ caps.system_file }} characters per `system/` file, {{ caps.injection }} for the whole compiled injection. Crossing either blocks the end of the turn. The escape is never truncation -- move detail to a `reference/` file and leave a `[label](/path)` link.
